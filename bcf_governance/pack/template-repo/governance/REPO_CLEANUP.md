...
3. Index retained history or declare git history as the retention source.
4. Use `bcf install --target . --force-rescaffold` only after accepting the destructive warning.
5. Tune architecture gates to the repo's real layout; do not delete gates to make validation pass.
6. Review README, docs, runbooks, plans, and phase logs section by section against repo evidence.
7. Record command outcomes and unresolved constraints in the active phase log.

## Deterministic Work

BCF can move audit/review evidence into `audits/`, create `audits/README.md`,
rewrite exact path references, and reinstall known BCF-owned files.

## LLM Or Human Review

Use judgment for documentation currency, product specs, architecture/security
docs, runbooks, phase history compaction, abandoned YAML, and nested governance.
