...
def _projection_receipt(
    ...
    raw = api.artifact_bytes(repository, artifact["id"])
    if f"sha256:{hashlib.sha256(raw).hexdigest()}" != artifact["digest"]:
        raise GitHubControllerError("recovery receipt artifact bytes do not match provider digest")
    with tempfile.TemporaryDirectory(prefix="bcf-recovery-reentry-receipt-") as name:
        ...
        if [path.relative_to(extracted).as_posix() for path in files] != ["receipt.json"]:
    ...
    if (
        ...
        or str(probe_run.get("repository", {}).get("id")) != receipt["repository"]["id"]
    ):
...
def project_installation(
    *,
    ...
    if (
        ...
        raise GitHubControllerError("recovery receipt controller confirmation is contradictory")
    first_projection = current.get("installed_commit_sha") == pin.get("BCF_BOOTSTRAP_COMMIT_SHA")
    reentry_projection = current == confirmation and current.get("installed_commit_sha") != pin.get("BCF_BOOTSTRAP_COMMIT_SHA")
    if not first_projection and not reentry_projection:
        raise GitHubControllerError("controller custody cannot admit recovery re-entry projection")
    ...
    authorization = _reentry_authorization(
        root=root,
        receipt=receipt,
    ...
    existing_authorization = runner.get("trusted_controller_recovery_reentry")
    if existing_authorization is not None and (
        not isinstance(existing_authorization, dict)
        or existing_authorization.get("operation_id") != authorization["operation_id"]
        or existing_authorization.get("authorized_source")
        != authorization["authorized_source"]
        or existing_authorization.get("installed_controller_commit")
    ...
    installation_line = f"  trusted_controller_installation: {confirmation}".encode()
