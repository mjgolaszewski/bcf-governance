...
def _normalize_workitem_statuses(
    entries: object, legacy: dict[str, Any], *, prefix: str
) -> bool:
    ...
    for index, raw in enumerate(entries):
        if not isinstance(raw, dict) or not isinstance(raw.get("status"), str):
            continue
        old = raw["status"]
        if old in {"TODO", "IN_PROGRESS", "BLOCKED", "DONE"}:
            continue
        canonical = WORKITEM_STATUS_ALIASES.get(old.strip().lower().replace("-", "_"))
        if canonical is None:
            raise ValueError(f"cannot migrate unknown workitem status {old!r} at {prefix}.{index}")
        legacy[f"{prefix}.{index}.status"] = old
...
def _migrate_phase_log(
    ...
    _normalize_workitem_statuses(payload.get("workitems"), legacy, prefix="workitems")
...
def _migrate_workitems(
    ...
    changed = _normalize_workitem_statuses(entries, legacy, prefix="workitems")
    if isinstance(entries, list):
        for raw in entries:
...
def apply_migration_transaction(repo_root: Path) -> dict[str, Any]:
    """Apply a previewed migration through the shared rollback-safe engine."""
    repo_root = repo_root.resolve()
    report = migration_plan(repo_root, apply=False)
        ...
        dict.fromkeys(
            [
                *report["changed_paths"],
                "governance/migrations/evidence-integrity-v1.yml",
            ]
        )
    )

    def mutate(shadow: Path) -> None:
        migration_plan(shadow, apply=True)
        if migration_plan(shadow, apply=False)["changed_paths"]:
            raise RuntimeError("evidence migration did not reach an idempotent state")
...
def main(argv: list[str] | None = None) -> None:
    ...
    report = (
        apply_migration_transaction(args.repo_root)
        if args.apply
        else migration_plan(args.repo_root, apply=False)
    )
    if args.format == "json":
