...
Review the audit whenever a graph, gate, test partition, artifact, authority role,
dependency surface, or timeout changes. Gate subprocess deadlines belong to the
gate registry, and evidence code consumes them directly. The graph's outer job
...
renderer projects its credential references and least-privilege App token
permissions. Candidate jobs may prepare an input but cannot publish, retain,
